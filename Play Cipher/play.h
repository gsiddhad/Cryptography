...
public:
	...
	char* Encrypt(char* plain, char keymat[5][5]) {
		...
		for (i = 0; i < strlen(plain); i += 2) {
			...
			if (mat[ch[0]][0] != mat[ch[1]][0]
					&& mat[ch[0]][1] != mat[ch[1]][1]) {
				cipher[i] = (char) keymat[mat[ch[0]][0]][mat[ch[1]][1]];
				cipher[i + 1] = (char) keymat[mat[ch[1]][0]][mat[ch[0]][1]];
			} else if (mat[ch[0]][0] == mat[ch[1]][0]) {    // Test 2 (Same Row)
				cipher[i] =
						(char) keymat[mat[ch[0]][0]][(mat[ch[0]][1] + 1) % 5];
				cipher[i + 1] = (char) keymat[mat[ch[1]][0]][(mat[ch[1]][1] + 1)
						% 5];
			} else if (mat[ch[0]][1] == mat[ch[1]][1]) { // Test 3 (Same Column)
				cipher[i] =
